 ...
 * @since    1.0.0
 */
class IG_Contactform_Post_Type {
		...
		if ( in_array( $column, array( 'ip', 'browser', 'os' ) ) ) {
			...
			if ( ! empty( $formID ) && is_numeric( $formID ) ) {
				$submissionData = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}ig_contactform_submission_data WHERE form_id = %d AND field_id = 0 AND submission_id=%d AND field_type=%s ORDER BY submission_data_id ASC", (int)$formID, (int)$post_id, $column
					)
				);
			...
			$where = '';
			$total = $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->posts WHERE (post_status = 'publish' OR post_status = 'trash' OR post_status = 'draft' OR post_status = 'pending') AND (post_content = '" . (int)$form_id . "'  AND post_type = 'ig_cfsb_post_type' ) " . $where );
			return '<a href="edit.php?post_status=all&post_type=ig_cfsb_post_type&action=-1&m=0&paged=1&mode=list&action2=-1&ig_contactform_form_id=' . $form_id . '">' . $total . '</a>';
		}
			...
			return '[ig_contactform id=' . $form_id . ']';
		}
			...
			if ( ! empty( $formID ) && is_numeric( $formID ) && ! empty( $column ) && is_numeric( $column ) ) {
				$submissionData = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM {$wpdb->prefix}ig_contactform_submission_data WHERE form_id = %d AND field_id = %d AND submission_id=%d ORDER BY submission_data_id ASC", (int)$formID, (int)$column, (int)$post_id
					)
				);
				if ( ! empty( $submissionData[ 0 ] ) ) {
					$getData = IGContactformHelper::get_data_field( $submissionData[ 0 ]->field_type, $submissionData[ 0 ], '', $formID, true, true, 'list' );
					if ( $return ) {
						return $getData;
					}
					else {
