...
if ( ! empty( $this->items ) ) {
	foreach ( $this->items as $items ) {
		...
		if ( ! empty( $items[ 'options' ] ) ) {
			$output .= '<select class="ig-contactform-select2" name=\'contactform_action_data[' . $items[ 'name' ] . ']\'>';
			...
			$output .= __( 'IG_CONTACTFORM_SELECT_CHOICE_NO', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . str_replace(
				array( '_', '-' ), ' ', $items[ 'name' ]
			) . ' found';
		}
		$output .= '</div>';
