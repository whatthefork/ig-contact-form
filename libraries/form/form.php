...
class IG_Form {
	 ...
	 * @var  string
	 */
	protected $text_domain = IG_LIBRARY_TEXTDOMAIN;

	/**
	 * Instantiate a IG_Form object then return.
	 *
	...
	public static function get_instance( $form_id, $fields = array() ) {
		if ( ! isset( $instances[ $form_id ] ) ) {
			$instances[ $form_id ] = new IG_Form( $form_id, $fields );
		}
	...
	public function setup_form( $fields, $override = false ) {
		// Filter form fields
		$fields = apply_filters( 'ig_setup_form', $fields );
