...
if ( ! class_exists( 'IG_Product_Info' ) ) :
...
class IG_Product_Info {
	 ...
	 *
	 * @param   array    $addons        Either product add-ons category name or an array of product add-ons, e.g. array( 'ig-addon-1' => '1.0.0', 'ig-addon-2' => null ).
	...
	public static function addons( $addons, $product_info = array(), $results = array() ) {
		...
		if ( is_array( $addons ) ) {
			...
			// Get product add-ons information from product info retrieved from InnoGears server
			foreach ( $product_info->items as $item ) {
	 ...
	 *
	 * @param   string   $name      Plugin's identified name defined in InnoGears server.
