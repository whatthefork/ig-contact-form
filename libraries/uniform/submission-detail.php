 ...
 * @since    1.0.0
 */
class IG_Uniform_Submission_Detail {
	...
	public static function print_submission_detail_html( $post ) {
		$fieldsFormAction = self::ig_uniform_get_field();
		if ( is_object( $post ) ) {
		...
		// Init HTML form
		$form = IG_Form::get_instance( 'ig_uniform_submission_detail', $fieldsFormAction );
	 ...
	 * @return  void
	 */
	public static function ig_uniform_submission_save_form( $post_id ) {
		global $wpdb;
		if ( ! empty( $_POST[ 'submission' ] ) ) {
			foreach ( $_POST[ 'submission' ] as $id => $value ) {
				...
				$wpdb->update(
					$wpdb->prefix . 'ig_uniform_submission_data', array( 'submission_data_value' => stripslashes( $value ) ), array(
						'field_id' => $id,
