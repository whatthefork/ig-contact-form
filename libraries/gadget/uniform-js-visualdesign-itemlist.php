...
class IG_Gadget_Uniform_Js_Visualdesign_Itemlist extends IG_Gadget_Base {
	 ...
	 * @var  string
	 */
	protected $gadget = 'uniform-js-visualdesign-itemlist';
	...
	public function default_action() {
		...
		$template = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_template', $template );
		...
		$updateItems = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_update_items', $updateItems );

		$updateAction = '  var items = this.control.find(\'input[type="checkbox"],input[type="radio"]\');
            var listItems = {};
            $(".ig-action-active").remove();
            items.each(function (index, item) {
		...
		$updateAction = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_update_action', $updateAction );
		...
		$getBoxStyle = 'var display = element.css(\'display\')
            ...
            return style;';
		$getBoxStyle = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_get_box_style', $getBoxStyle );

		$openActionSettings = '$(".ig-lock-screen").remove();
            $("body").append($("<div/>", {
                "class":"ig-lock-screen",
            ...
            var container = $("#ig_uniform_master #form-container"), listOptionPage = [], instance = container.data(\'visualdesign-instance\'), content = "";
                ...
                $("<div/>", {
                    ...
                    $("<div/>", {
                        ...
                        $("<div/>", {"id":"accordion_content", "class":"hide"}).append(
                            ...
                            $("<div/>", {"id":"ig-action-show"})
                        ).append(
                            $("<h3/>").append(
                                $("<a/>", {"href":"#"}).append("Hide form fields(s)")
                            )
                        ).append(
                            $("<div/>", {"id":"ig-action-hide"})
                        )
                    )
                )
            )
                    ...
                    $.ajax({
                        ...
                        success:function (response) {
                            ...
                            if ($(listFieldShow).html()) {
                                $(".control-list-action #ig-action-show").append(listFieldShow);
                            } else {
                                $(".control-list-action #ig-action-show").append($("<ul/>", {"class":"jsn-items-list"}).append(
                                    $("<div/>", {"class":"ui-state-disabled"}).append(
                                        self.options.language.IG_UNIFORM_ALL_FORM_FIELD_ARE_DISPLAYED
                                    )
                                ));
                            }
                            if ($(listFieldHide).html()) {
                                $(".control-list-action #ig-action-hide").append(listFieldHide);
                            } else {
                                $(".control-list-action #ig-action-hide").append($("<ul/>", {"class":"jsn-items-list"}).append(
                                    $("<div/>", {"class":"ui-state-disabled"}).append(
                                        self.options.language.IG_UNIFORM_ALL_FORM_FIELD_ARE_HIDDEN
                                    )
                                ));
                            }
                            $("#ig-action-show input[type=checkbox]").change(function () {
                                var dataShowField = [];
                                $("#ig-action-show input[type=checkbox]:checked").each(function () {
                            ...
                            $("#ig-action-hide input[type=checkbox]").change(function () {
                                var dataHideField = [];
                                $("#ig-action-hide input[type=checkbox]:checked").each(function () {
                            ...
                            $(".control-list-action #accordion_content #ig-action-show,.control-list-action #accordion_content #ig-action-hide").css({\'height\':\'auto\'});
                            $("#action-loading").addClass("hide");
                            $("#accordion_content").removeClass("hide");
                            $(".ig-lock-screen").remove();
		...
		$openActionSettings = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_action_settings', $openActionSettings );

		$addEvents = ' var self = this;
            var listItems = [];
            var itemChecked = [];
            this.control.find(\'a.ig-ic-move\').click(function () {
                return false;
            });
            ...
            self.control.find("#items-list-save").click(function (e) {
                ...
                self.updateItems();
                self.control.find(\'a.ig-ic-move\').click(function () {
                    return false;
                });
		...
		$addEvents = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_add_events', $addEvents );
		$render = 'var self = this;
		            ...
		            this.updateAction();
		            return this.control;';
		$render = apply_filters( 'ig_uniform_filter_visualdesign_itemlist_render', $render );
