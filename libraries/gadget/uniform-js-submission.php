...
class IG_Gadget_Uniform_Js_Submission extends IG_Gadget_Base {
	 ...
	 * @var  string
	 */
	protected $gadget = 'uniform-js-submission';
	...
	public function default_action() {
		...
		$mainContent = apply_filters( 'ig_uniform_js_submission_main_content', $mainContent );
		...
		$createPrototypeSubmission[ 'init' ] = 'init:function () {
                                ...
                                $("#ig_uniform_submission_detail").hide();
					            ...
					            $($(submissionSettings).find("div.submission-page")[0]).show();
					            $("#ig_uniform_submission_detail .hndle,#ig_uniform_submission_detail .handlediv").click(function () {
					                setTimeout(function () {
					                    self.checkPage();
					                }, 200)
					            });
		...
		$actionCheckPage = apply_filters( 'ig_uniform_action_check_page_submission', $actionCheckPage );
		...
		$createPrototypeSubmission = apply_filters( 'ig_uniform_js_form_add_prototype_form', $createPrototypeSubmission );
		$javascript = '(function ($) {
					    var JSNUniformSubmissionView = function (params) {
					        ...
					        $("#menu-posts-ig_uniform_post_type").removeClass("wp-not-current-submenu").addClass("wp-has-current-submenu");
					        $("#menu-posts-ig_uniform_post_type > a").removeClass("wp-not-current-submenu").addClass("wp-has-current-submenu wp-menu-open");
					    ...
					    JSNUniformSubmissionView.prototype = { ' . implode( ',', $createPrototypeSubmission ) . '}
					    var params = {};
					    ' . $mainContent . '
					    new JSNUniformSubmissionView(params);
					})(jQuery);';
		echo '' . $javascript;
		exit();
	}
