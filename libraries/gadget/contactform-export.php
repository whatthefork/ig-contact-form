...
class IG_Gadget_Contactform_Export extends IG_Gadget_Base {
	...
	public function task_export() {
		...
		if ( $formId && $fieldExport ) {
			...
			$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_cfsb_post_type' ORDER BY id ASC", (int)$formId );
			if ( ! empty( $dateSubmission ) ) {
				...
				if ( @$dateSubmission[ 1 ] ) {
					...
					$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_cfsb_post_type' AND ( date(post_date) BETWEEN %s AND %s )  ORDER BY id ASC", (int)$formId, $dateStart, $dateEnd );
				}
				else {
					$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_cfsb_post_type' AND date(submission_created_at) = %s  ORDER BY id ASC", (int)$formId, $dateStart );
				}
			}
			...
			foreach ( $submissions as $submission ) {
				...
				foreach ( $fieldExport as $fx ) {
					if ( $fx == 'date_created' ) {
						...
						$submissionData[ ] = IG_Contactform_Post_Type::render_submissions_column( $fx, $submission->id, true, $formId );
					}
				}
				$data[ ] = $submissionData;
			}
			if ( isset( $_POST[ 'exportType' ] ) && $_POST[ 'exportType' ] == 'excel' ) {
				include_once ( IG_CONTACTFORM_PATH . 'libraries/3rd-party/php-excel.class.php' );
				// generate file (constructor parameters are optional)
				$xls = new Excel_XML( 'UTF-8', false, 'My Test Sheet' );
				$xls->addArray( $data );
				$xls->generateXML( 'ig-contactform-' . $_POST[ 'form_title' ] . '-excel-' . date( 'Y-m-d' ) );
				exit();
			}
			else if ( isset( $_POST[ 'exportType' ] ) && $_POST[ 'exportType' ] == 'csv' ) {
				$fileName = 'ig-contactform-' . $_POST[ 'form_title' ] . '-csv-' . date( 'Y-m-d' );
