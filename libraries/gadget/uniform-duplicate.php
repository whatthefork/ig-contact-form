...
class IG_Gadget_Uniform_Duplicate extends IG_Gadget_Base {
	 ...
	 * @var  string
	 */
	protected $gadget = 'uniform-duplicate';
	...
	public function duplicate( $form_id ) {
		...
		if ( ! empty( $dataForm ) ) {
			...
			foreach ( $getPostMeta as $postMeta ) {
				if ( $postMeta->meta_key == 'form_id' ) {
					...
					add_post_meta( $post_id, $postMeta->meta_key, $postMeta->meta_value );
				}
			}
			$getFormPages = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ig_uniform_form_pages WHERE  form_id=%d", (int)$form_id ) );
			...
			foreach ( $getFormPages as $page ) {
				...
				foreach ( $fields as $index => $item ) {
					...
					$wpdb->update( $wpdb->prefix . 'ig_uniform_fields', array( 'field_settings' => json_encode( $fieldSettings ) ), array( 'field_id' => $wpdb->insert_id ), array( '%s' ), array( '%d' ) );
		...
		header( 'Location: ' . get_admin_url() . 'edit.php?post_type=ig_uniform_post_type' );
	}
}
