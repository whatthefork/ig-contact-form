...
class IG_Gadget_Contactform_Frontend extends IG_Gadget_Base {
	...
	public function default_action() {
		if ( ! empty( $_GET[ 'task' ] ) && $_GET[ 'task' ] == 'form.save' && ! empty( $_GET[ 'form_id' ] ) ) {
			include_once ( IG_CONTACTFORM_PATH . 'frontend/helpers/browser.php' );
	...
	public function task_save() {
		...
		if ( @$_SERVER[ 'CONTENT_LENGTH' ] < (int)( ini_get( 'post_max_size' ) ) * 1024 * 1024 ) {
			...
			echo '<input type="hidden" name="error" value=\'' . htmlentities( json_encode( array( 'max-upload' => __( 'IG_CONTACTFORM_POST_MAX_SIZE', IG_CONTACTFORM_TEXTDOMAIN ) ) ), ENT_QUOTES, 'UTF-8' ) . '\'/>';
			exit();
		}
	}
	...
	private function get_action_form( $formAction, $formData, &$return ) {
		...
		$getReturn = apply_filters( 'ig_contactform_frontend_get_action_form', $return );
		if ( ! empty( $getReturn ) ) {
			$getReturn = $return;
		}
	}
	...
	public static function field_json( $post, $fieldType, $fieldIdentifier ) {
		...
		$checkFieldJson = apply_filters( 'ig_contactform_frontend_check_field_json', $checkFieldJson );
	...
	public static function check_duplicates( $post, $fieldIdentifier, $fieldTitle, &$validationForm ) {
		...
		if ( $postIndentifier ) {
			...
			$columsSubmission = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT submission_id FROM {$wpdb->prefix}ig_contactform_submission_data WHERE form_id = %d AND submission_data_value=%s", (int)$formId, $postIndentifier
				)
			);

			if ( $columsSubmission ) {
				$error = __( 'IG_CONTACTFORM_FIELD_EXISTING', IG_CONTACTFORM_TEXTDOMAIN );
	...
	public static function check_limit_char( $post, $fieldSettings, $fieldIdentifier, $fieldTitle, &$validationForm ) {
		...
		if ( isset( $fieldSettings->options->limitType ) && $fieldSettings->options->limitType == 'Words' ) {
			...
			if ( count( $countValue ) < $fieldSettings->options->limitMin ) {
				$validationForm[ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MIN_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMin . ' Words';
			}
			else if ( count( $countValue ) > $fieldSettings->options->limitMax ) {
				$validationForm[ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MAX_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMax . ' Words';
			}
		}
		else {
			if ( isset( $fieldSettings->type ) && $fieldSettings->type != 'password' ) {
				if ( strlen( $postIndentifier ) < $fieldSettings->options->limitMin ) {
					$validationForm[ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MIN_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMin . ' Character';
				}
				else if ( strlen( $postIndentifier ) > $fieldSettings->options->limitMax ) {
					$validationForm[ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MAX_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMax . ' Character';
				}
			}
			else {
				if ( strlen( $postIndentifier ) < $fieldSettings->options->limitMin ) {
					$validationForm[ 'password' ][ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MIN_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMin . ' Character';
				}
				else if ( strlen( $postIndentifier ) > $fieldSettings->options->limitMax ) {
					$validationForm[ 'password' ][ $fieldIdentifier ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_MAX_LENGTH', IG_CONTACTFORM_TEXTDOMAIN ) . ' ' . $fieldSettings->options->limitMax . ' Character';
				}
			}
		}
	}
	...
	public function save( $post, $postID ) {
		...
		if ( ! empty( $formSettings->form_captcha ) ) {
			if ( $formSettings->form_captcha == 1 ) {
				include_once ( IG_CONTACTFORM_PATH . 'libraries/3rd-party/recaptchalib.php' );
				...
				$resp = recaptcha_check_answer( IG_CONTACTFORM_CAPTCHA_PRIVATEKEY, $_SERVER[ 'REMOTE_ADDR' ], $recaptchaChallenge, $recaptchaResponse );

				if ( ! $resp->is_valid ) {
					$return->error[ 'captcha' ] = __( 'IG_CONTACTFORM_ERROR_CAPTCHA', IG_CONTACTFORM_TEXTDOMAIN );
				...
				if ( ! empty( $_POST[ 'form_name' ] ) && ! empty( $_POST[ 'captcha' ] ) ) {
					...
					if ( strtolower( $sCaptcha ) != strtolower( $_POST[ 'captcha' ] ) ) {
						$return->error[ 'captcha_2' ] = __( 'IG_CONTACTFORM_ERROR_CAPTCHA', IG_CONTACTFORM_TEXTDOMAIN );
						return $return;
					}
				}
				else {
		...
		$columsSubmission = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}ig_contactform_fields WHERE form_id = %d ORDER BY field_ordering ASC", (int)$postFormId
			)
		);
		...
		if ( isset( $dataForms->form_type ) && $dataForms->form_type == 1 ) {
			$dataPages = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}ig_contactform_form_pages WHERE form_id = %d ORDER BY page_id ASC", (int)$dataForms[ 'form_id' ]
				)
			);
		...
		foreach ( $columsSubmission as $colum ) {
			...
			if ( ! in_array( $colum->field_id, $fieldClear ) ) {
				...
				$formTypeNotSave = apply_filters( 'ig_contactform_filter_form_type_not_save', $formTypeNotSave );
				...
				if ( isset( $colum->field_type ) && ! in_array( $colum->field_type, $formTypeNotSave ) ) {
					...
					elseif ( in_array( $colum->field_type, array( 'checkboxes', 'list' ) ) ) {
						...
						$getValue = '';
						$getValue = apply_filters( 'ig_contactform_get_value_type_' . str_replace( '-', '_', $colum->field_type ), $post, $fieldName, $colum, $fieldSettings );
					...
					if ( isset( $colum->field_type ) ) {
						...
						$contentField = IGContactformHelper::get_data_field( $colum->field_type, $submissions, $colum->field_id, $postFormId, false, false, 'email' );
						/* Create Filter get file attachment*/
						$fileAttach = apply_filters( 'ig_contactform_frontend_file_attachment_email', $fileAttach, $colum, $submissions, $postFormId );
					...
					if ( ! empty( $fieldSettings->options->noDuplicates ) && (int)$fieldSettings->options->noDuplicates == 1 ) {
						IG_Gadget_Contactform_Frontend::check_duplicates( $post, $fieldName, $colum->field_title, $validationForm );
					}
					...
					if ( isset( $fieldSettings->options->requiredConfirm ) && (int)$fieldSettings->options->requiredConfirm == 1 ) {
						...
						if ( isset( $fieldSettings->options->required ) && (int)$fieldSettings->options->required == 1 && $postData != $postDataConfirm ) {
							$error = __( 'IG_CONTACTFORM_CONFIRM_FIELD_CONFIRM', IG_CONTACTFORM_TEXTDOMAIN );
					...
					if ( isset( $fieldSettings->options->required ) && (int)$fieldSettings->options->required == 1 && (int)$fieldSettings->options->hideField != 1 ) {
						...
						$checkValidation = apply_filters( 'ig_contactform_filter_required_type_' . str_replace( '-', '_', $colum->field_type ), $checkValidation, $post, $fieldName, $colum, $fieldSettings );
						if ( ! empty( $checkValidation ) ) {
							...
							if ( isset( $post[ $fieldName ] ) && $post[ $fieldName ] == '' ) {
								$validationForm[ $fieldName ] = __( 'IG_CONTACTFORM_CONFIRM_FIELD_CANNOT_EMPTY', IG_CONTACTFORM_TEXTDOMAIN );
							}
						}
					}
					do_action( 'ig_contactform_frontend_action_save_form', $validationForm, $submissions, $postFormId, $fieldSettings, $post, $fieldName, $colum );
					$validationForm = apply_filters( 'ig_contactform_frontend_validation_save_form', $validationForm, $colum, $submissions, $postFormId, $fieldSettings, $post, $fieldName );
				}
				else {
					$formTypeNotSendEmail = array();
					$formTypeNotSendEmail = apply_filters( 'ig_contactform_filter_form_type_not_send_email', $formTypeNotSendEmail );
	...
	private function _save( $dataForms, $postID, &$return, $post, $submissionsData, $dataContentEmail, $nameFileByIndentifier, $requiredField, $fileAttach ) {
		global $wpdb;
		do_action( 'ig_contactform_before_save_form', $dataForms, $postID, $post, $submissionsData, $dataContentEmail, $nameFileByIndentifier, $requiredField, $fileAttach );
		...
		if ( $checkSaveSubmission ) {
			foreach ( $submissionsData as $submission ) {
				if ( ! empty( $submission ) ) {
					...
					if ( in_array( $submission[ 'field_type' ], array( 'os', 'browser', 'ip', 'date' ) ) ) {
						$wpdb->insert(
							$wpdb->prefix . 'ig_contactform_submission_data', $submission, array( '%d', '%s', '%s', '%d' )
						);
					}
					else {
						$wpdb->insert(
							$wpdb->prefix . 'ig_contactform_submission_data', $submission, array(
								'%d',
								'%d',
								'%s',
								'%s',
			...
			$countPost = wp_count_posts( 'ig_cfsb_post_type' );
		...
		do_action( 'ig_contactform_after_save_form', $dataForms, $postID, $post, $submissionsData, $dataContentEmail, $nameFileByIndentifier, $requiredField, $fileAttach );
		return true;
	}
	...
	function get_html_form( $formId ) {
		if ( ! empty( $formId ) && is_numeric( $formId ) ) {
			...
			echo '' . IGContactformHelper::generate_html_pages( $postId, $formId, $formName, 'ajax' );
			exit();
		}
	}
}
