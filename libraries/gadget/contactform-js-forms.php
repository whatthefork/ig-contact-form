...
class IG_Gadget_Contactform_Js_Forms extends IG_Gadget_Base {
		...
		$jsHook['button-addnew-action'] = '$("#wpbody-content .jsn-form-title-heading h2").after(
			            $("<div/>", {"class":"contactform-add-new"}).append(
			                ...
			                $("<ul/>", {"class":"contactform-sample-form"}).append(
			                    $("<li/>").append(
			                        $("<a/>", {"class":"", "href":"post-new.php?post_type=ig_cf_post_type", "text":"Blank Form"})
			                    )
			                )
			            )
			        );';
		$jsHook = apply_filters( 'ig_contactform_js_forms_hook', $jsHook );
		$javascript = '(function ($) {
			    $(function () {
