...
class IG_Gadget_Contactform_Js_Submissions extends IG_Gadget_Base {
	...
	public function default_action() {
		...
		$mainContent = apply_filters( 'ig_contactform_js_submissions_main_content', $mainContent );
		...
		if ( ! empty( $forms ) ) {
			foreach ( $forms as $f ) {
				...
				$fielForm = IGContactformHelper::get_filed_by_form_id( $fID );
				if ( ! empty( $fielForm ) ) {
					$column = array();
		...
		$javascript = '(function ($) {
				    ...
				    $(".jsn-modal-overlay,.jsn-modal-indicator").remove();
				        ...
				        if( $("#the-list>.no-items").length>0 && parseInt(count)<1){
							$("#btn-contactform-export,#ig-submission-filter-date,#clear-submit,.actions.bulkactions").remove();
				        ...
				        $("#btn-contactform-export").after(
				            $("<div/>", {
				                ...
				                $("<div/>", {
				                    ...
				                    $("<form/>", {"action":"?ig-gadget=contactform-export&action=default", "method":"post", "id":"contactform_form_export"}).append(exportField).append(
				        ...
				        $("#dialog-export").dialog({
				            ...
				            buttons:{
				                Export:function () {
				                    ...
				                    $("#dialog-export #contactform_form_export #form_title").val($("#dropdown_ig_form_id option:selected").text());
				                    $("#dialog-export #contactform_form_export").submit();

				                    //  $.post("?ig-gadget=contactform-export&action=default", {\'form_id\':formID, \'task\':\'contactform.export\', \'fieldExport\':fieldExport, \'exportType\':exportType, \'form_title\':$("#dropdown_ig_form_id option:selected").text()});
				                },
				                Close:function () {
				                    $(this).dialog("close");
				                }
				        ...
				        $(".container-export").before(
				           $("<div/>",{"class":"row-fluid ig-contactform-export-option"}).append(
			                    ...
			                    $("<div/>",{"class":"span4"}).append(
			                             $("<div/>", {"class":"ig-contactform-filter-date pull-right"}).append(
				        ...
				        $("#adv-settings .metabox-prefs label input[type=checkbox]").change(function(){
				           ...
				           $("#adv-settings .metabox-prefs label input[type=checkbox]:checked").each(function(){
							 ...
							 $.ajax({
	                            type:"POST",
	                            dataType:"json",
	                            url:"admin-ajax.php?action=ig_contactform_hidden_columns",
	                            data:{
	                                columns:columns,
