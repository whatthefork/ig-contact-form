...
class IG_Gadget_Uniform_Export extends IG_Gadget_Base {
	 ...
	 * @var  string
	 */
	protected $gadget = 'uniform-export';
	...
	public function default_action() {
		...
		auth_redirect();
		if ( ! empty( $_POST[ 'task' ] ) && $_POST[ 'task' ] == 'uniform.export' && ! empty( $_POST[ 'form_id' ] ) ) {
			self::task_export();
		}
	...
	public function task_export() {
		...
		if ( $formId && $fieldExport ) {
			...
			$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_uniform_sb' ORDER BY id ASC", (int)$formId );
			if ( ! empty( $dateSubmission ) ) {
				...
				if ( @$dateSubmission[ 1 ] ) {
					...
					$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_uniform_sb' AND ( date(post_date) BETWEEN %s AND %s )  ORDER BY id ASC", (int)$formId, $dateStart, $dateEnd );
				}
				else {
					$query = $wpdb->prepare( "SELECT id,post_date FROM {$wpdb->posts} WHERE post_content = %d AND post_type = 'ig_uniform_sb' AND date(submission_created_at) = %s  ORDER BY id ASC", (int)$formId, $dateStart );
				}
			}
			...
			foreach ( $submissions as $submission ) {
				...
				foreach ( $fieldExport as $fx ) {
					if ( $fx == 'date_created' ) {
						...
						$submissionData[ ] = IG_Uniform_Post_Type::render_submissions_column( $fx, $submission->id, true, $formId );
					}
				}
				$data[ ] = $submissionData;
			}
			if ( isset( $_POST[ 'exportType' ] ) && $_POST[ 'exportType' ] == 'excel' ) {
				include_once ( IG_UNIFORM_PATH . 'libraries/3rd-party/php-excel.class.php' );
				// generate file (constructor parameters are optional)
				$xls = new Excel_XML( 'UTF-8', false, 'My Test Sheet' );
				$xls->addArray( $data );
				$xls->generateXML( 'ig-uniform-' . $_POST[ 'form_title' ] . '-excel-' . date( 'Y-m-d' ) );
				exit();
			}
			else if ( isset( $_POST[ 'exportType' ] ) && $_POST[ 'exportType' ] == 'csv' ) {
				$fileName = 'ig-uniform-' . $_POST[ 'form_title' ] . '-csv-' . date( 'Y-m-d' );
