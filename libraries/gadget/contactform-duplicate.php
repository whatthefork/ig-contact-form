...
class IG_Gadget_Contactform_Duplicate extends IG_Gadget_Base {
	...
	public function duplicate( $form_id ) {
		...
		if ( ! empty( $dataForm ) ) {
			...
			$getFormPages = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}ig_contactform_form_pages WHERE  form_id=%d", (int)$form_id ) );
			...
			foreach ( $getFormPages as $page ) {
				...
				foreach ( $fields as $index => $item ) {
					...
					$wpdb->update( $wpdb->prefix . 'ig_contactform_fields', array( 'field_settings' => json_encode( $fieldSettings ) ), array( 'field_id' => $wpdb->insert_id ), array( '%s' ), array( '%d' ) );
		...
		header( 'Location: ' . get_admin_url() . 'edit.php?post_type=ig_cf_post_type' );
	}
}
