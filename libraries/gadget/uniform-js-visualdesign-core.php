...
class IG_Gadget_Uniform_Js_Visualdesign_Core extends IG_Gadget_Base {
	 ...
	 * @var  string
	 */
	protected $gadget = 'uniform-js-visualdesign-core';
	...
	public function default_action() {
		...
		$addParamsVisualDesign = array( 'newElement' => 'this.newElement = $(\'<a href="javascript:void(0);" class="jsn-add-more"><i class="icon-plus"></i>\' + lang[\'IG_UNIFORM_ADD_FIELD\'] + \'</a>\');' );
		/* Create Filter add params visual design */
		$addParamsVisualDesign = apply_filters( 'ig_uniform_visualdesign_add_params', $addParamsVisualDesign );
		$addBoxContent = array();
		...
		$addBoxContent = apply_filters( 'ig_uniform_visualdesign_add_box_content', $addBoxContent );

		/* Create Filter get action change option content */
		$actionChangeOptionsBoxContent = array();
		$actionChangeOptionsBoxContent = apply_filters( 'ig_uniform_visualdesign_action_change_option_box_content', $actionChangeOptionsBoxContent );

		/* Create Filter get action change option box content */
		$beforeActionChangeOptionBoxContent = array();
		$beforeActionChangeOptionBoxContent = apply_filters( 'ig_uniform_visualdesign_before_action_change_option_box_content', $beforeActionChangeOptionBoxContent );
		/* Create Filter get after action change option box content */
		$afterActionChangeOptionBoxContent = array();
		$afterActionChangeOptionBoxContent = apply_filters( 'ig_uniform_visualdesign_before_action_change_option_box_content', $afterActionChangeOptionBoxContent );
		...
		$eventBoxContentSubmit = apply_filters( 'ig_uniform_visualdesign_event_box_content_submit', $eventBoxContentSubmit );
		...
		$ifCheckTargetMouseDownBoxContent = apply_filters( 'ig_uniform_visualdesign_if_check_target_mousedown_box_content', $ifCheckTargetMouseDownBoxContent );
		...
		$logicIfCheckTargetMouseDownBoxContent = apply_filters( 'ig_uniform_visualdesign_logic_if_check_target_mousedown_box_content', $logicIfCheckTargetMouseDownBoxContent );
		...
		$eventMouseDownBoxContent = apply_filters( 'ig_uniform_visualdesign_event_mousedown_box_content', $eventMouseDownBoxContent );
		...
		$ifCheckRenderButtonAddField = apply_filters( 'ig_uniform_visualdesign_if_check_render_button_add_field', $ifCheckRenderButtonAddField );
		...
		$eventClickButtonAddField = apply_filters( 'ig_uniform_visualdesign_event_click_button_add_field', $eventClickButtonAddField );
		...
		$getFilterAddButtonField = apply_filters( 'ig_uniform_add_select_button_field', $filterAddButtonField );
		...
		$filterButtonField[ 'extra' ] = ' case \'extra\':
				                        ...
				                        break;';
		$getFilterButtonField = apply_filters( 'ig_uniform_button_field', $filterButtonField );
				           ...
				           ' . implode( ' ', $filterAddButtonField ) . '
				            ...
				            JSNVisualDesign.toolbox.find("form").append(
				                ...
				                $("<div/>",{"class":"row-fluid"}).append(
				                    $("<span/>",{"class":"jsn-uniform-more-addons"}).append("Want to add more fields? <a target=\"_blank\" title=\"Check add-ons\" href=\"edit.php?post_type=ig_uniform_post_type&page=ig-uniform-addons\">Check add-ons</a>.")
				                )
				                )
				            )
		...
		$createFunctionVisualDesign[ 'savePage' ] = '  JSNVisualDesign.savePage = function (action) {
				        var container = $("#ig_uniform_master #form-container");
				        var listOptionPage = [];
				        var listContainer = [];
				                ...
				                if (check == 0 || !check) {
				                    $("#email .email-submitters .jsn-items-list").append(
				                        $("<div/>", {
				                            ...
				                            "text":lang["IG_UNIFORM_NO_EMAIL"],
				                            "title":lang["IG_UNIFORM_NO_EMAIL_DES"]
				                        }))
				                }
		...
		$actionFunctionCreateElement = apply_filters( 'ig_uniform_visualdesign_action_function_create_element', $actionFunctionCreateElement );
		...
		$actionTypeCheckBoxCreateControl = apply_filters( 'ig_uniform_visualdesign_action_type_checkbox_create_control', $actionTypeCheckBoxCreateControl );
		...
		$actionOpenOptionsBox = apply_filters( 'ig_uniform_visualdesign_action_open_options_box', $actionOpenOptionsBox );
		...
		$prototypeSerialize = apply_filters( 'ig_uniform_visualdesign_prototype_serialize', $prototypeSerialize );
		$createFunctionVisualDesign = apply_filters( 'ig_uniform_visualdesign_create_function', $createFunctionVisualDesign );
		$containerFunction = array();
		$getContainerFunction = apply_filters( 'ig_uniform_visualdesign_container_function', $containerFunction );
		...
		$javascript = '(function ($) {
				    ...
				    JSNVisualDesign.openOptionsBox = function (sender, type, params, action) {
				        ...
				        $("#option-firstItemAsPlaceholder-checkbox").after(\'<i class="icon-question-sign" original-title="\' + lang["IG_UNIFORM_SET_ITEM_PLACEHOLDER_DES"] + \'"></i>\');
