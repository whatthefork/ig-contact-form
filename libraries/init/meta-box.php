...
class IG_Init_Meta_Box {
	...
	public static function add_meta_boxes() {
		...
		self::$boxes = apply_filters( 'ig_meta_box_register', self::$boxes );
	...
	public static function print_html( $id, $post ) {
		...
		if ( isset( self::$boxes[ $id ] ) ) {
			...
			wp_nonce_field( "ig_meta_box_{$id}", "ig_meta_box_{$id}_nonce" );
			...
			if ( isset( self::$boxes[ $id ]['callback'] ) && ! empty( self::$boxes[ $id ]['callback'] ) ) {
				...
				$html = '<input type="text" id="ig_meta_box_' . $id . '" name="ig_meta_box[' . $id . ']" value="' . esc_attr( $value ) . '" />';
			}
		}
	...
	public static function save_post( $id, $post_id ) {
		...
		if ( isset( self::$boxes[ $id ] ) ) {
			...
			if ( ! isset( $_POST["ig_meta_box_{$id}_nonce"] ) ) {
				return;
			}

			// Verify nonce data
			if ( ! wp_verify_nonce( $_POST["ig_meta_box_{$id}_nonce"], "ig_meta_box_{$id}" ) ) {
				return;
			}
			...
			if ( isset( self::$boxes[ $id ]['save_post'] ) && ! empty( self::$boxes[ $id ]['save_post'] ) ) {
				...
				$data = sanitize_text_field( $_POST['ig_meta_box'][ $id ] );
			}
